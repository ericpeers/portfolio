...
func magsHoldings() []providers.ParsedETFHolding {
	return []providers.ParsedETFHolding{
		{Ticker: "n/a", Name: "NVIDIA CORP SWAP", Percentage: 0.0886},
		{Ticker: "FGXXX", Name: "FIRST AMERICAN GOVERNMENT OBLIGS X", Percentage: 0.0721},
		{Ticker: "n/a", Name: "ALPHABET INC SWAP GS", Percentage: 0.0589},
		{Ticker: "n/a", Name: "AMAZON.COM INC SWAP", Percentage: 0.0576},
		{Ticker: "AMZN", Name: "AMAZON.COM INC", Percentage: 0.0562},
		{Ticker: "n/a", Name: "ALPHABET INC-CL A SWAP", Percentage: 0.0534},
		{Ticker: "NVDA", Name: "NVIDIA CORP", Percentage: 0.0533},
		{Ticker: "n/a", Name: "MICROSOFT CORP SWAP", Percentage: 0.0532},
		{Ticker: "MSFT", Name: "MICROSOFT CORP", Percentage: 0.0526},
		{Ticker: "TSLA", Name: "TESLA INC", Percentage: 0.0522},
		{Ticker: "META", Name: "META PLATFORMS INC CLASS A", Percentage: 0.0513},
		{Ticker: "n/a", Name: "META PLATFORMS INC-CLASS A SWAP", Percentage: 0.0512},
		{Ticker: "AAPL", Name: "APPLE INC", Percentage: 0.0503},
		{Ticker: "n/a", Name: "TESLA INC SWAP", Percentage: 0.0445},
		{Ticker: "n/a", Name: "APPLE INC SWAP", Percentage: 0.0442},
		{Ticker: "n/a", Name: "TESLA INC SWAP GS", Percentage: 0.0423},
		{Ticker: "GOOGL", Name: "ALPHABET INC CLASS A", Percentage: 0.0415},
		{Ticker: "n/a", Name: "APPLE INC SWAP GS", Percentage: 0.0409},
		{Ticker: "n/a", Name: "AMAZON INC SWAP GS", Percentage: 0.0358},
		{Ticker: "n/a", Name: "MICROSOFT CORP SWAP GS", Percentage: 0.0344},
		{Ticker: "n/a", Name: "META PLATFORMS INC SWAP GS", Percentage: 0.0342},
		{Ticker: "n/a", Name: "US DOLLARS", Percentage: 0.0053},
		{Ticker: "n/a", Name: "OTHER ASSETS AND LIABILITIES", Percentage: -0.0257},
...
func TestResolveSwapHoldings_MAGSData(t *testing.T) {
	...
	for _, h := range resolved {
		resolvedMap[h.Ticker] = h.Percentage
	}
...
func TestResolveSwapHoldings_OrphanedSwap(t *testing.T) {
	...
	holdings := []providers.ParsedETFHolding{
		{Ticker: "NVDA", Name: "NVIDIA CORP", Percentage: 0.50},
		{Ticker: "AAPL", Name: "APPLE INC", Percentage: 0.40},
		{Ticker: "n/a", Name: "MICROSOFT CORP SWAP", Percentage: 0.10},
	}
...
func TestNormalizeHoldings_ScalesUp(t *testing.T) {
	...
	holdings := []providers.ParsedETFHolding{
		...
		{Ticker: "MSFT", Name: "MICROSOFT CORP", Percentage: 0.25},
	}
...
func makeKnown(symbols ...string) map[string][]*models.SecurityWithCountry {
	...
	for _, s := range symbols {
		m[s] = []*models.SecurityWithCountry{{Security: models.Security{Ticker: s}}}
	}
	return m
}
...
func TestResolveSymbolVariants_DotToDash(t *testing.T) {
	...
	holdings := []providers.ParsedETFHolding{
		...
		{Ticker: "AAPL", Name: "Apple Inc", Percentage: 0.90},
	}
	result := services.ResolveSymbolVariants(holdings, known)
	if result[0].Ticker != "BRK-B" {
		t.Errorf("expected BRK.B → BRK-B, got %q", result[0].Ticker)
	}
	if result[1].Ticker != "AAPL" {
		t.Errorf("expected AAPL unchanged, got %q", result[1].Ticker)
	}
}
...
func TestResolveSymbolVariants_StrippedFallback(t *testing.T) {
	...
	result := services.ResolveSymbolVariants(holdings, known)
	if result[0].Ticker != "BRKB" {
		t.Errorf("expected BRK.B → BRKB, got %q", result[0].Ticker)
	}
}
...
func TestResolveSymbolVariants_NoPunctuationPassthrough(t *testing.T) {
	...
	result := services.ResolveSymbolVariants(holdings, known)
	if result[0].Ticker != "UNKN" {
		t.Errorf("expected UNKN unchanged, got %q", result[0].Ticker)
	}
}
...
func TestResolveSymbolVariants_NoMatchPassthrough(t *testing.T) {
	...
	known := makeKnown("AAPL")
	holdings := []providers.ParsedETFHolding{
		{Ticker: "BRK.X", Name: "Unknown", Percentage: 1.0},
	}
	result := services.ResolveSymbolVariants(holdings, known)
	if result[0].Ticker != "BRK.X" {
		t.Errorf("expected BRK.X unchanged when no match, got %q", result[0].Ticker)
	}
}
...
func TestResolveSpecialSymbols_USDCash(t *testing.T) {
	...
	if resolved[0].Ticker != "US DOLLAR" {
		t.Errorf("expected symbol US DOLLAR, got %q", resolved[0].Ticker)
	}
...
func TestResolveSpecialSymbols_CashCollateralUSD(t *testing.T) {
	holdings := []providers.ParsedETFHolding{
		...
		{Ticker: "n/a", Name: "CASH COLLATERAL EUR HBCFT", Percentage: 0.01},
	}
...
func TestResolveSpecialSymbols_MixedAccumulation(t *testing.T) {
	holdings := []providers.ParsedETFHolding{
		...
		{Ticker: "n/a", Name: "OTHER ASSETS AND LIABILITIES", Percentage: -0.01},
	}
